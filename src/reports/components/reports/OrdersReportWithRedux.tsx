...
export function OrdersReportWithRedux() {
  const dispatch = useAppDispatch();
  const reportRef = useRef<HTMLDivElement>(null);

  // Get data from Redux store
  const { orders, statusFilter, priorityFilter, loading, error } = useAppSelector(
    (state) => state.reports.orders
  );
  const dateRange = useAppSelector((state) => state.reports.filters.dateRange);

  // Local state for selected filters (UI only)
  const [selectedStatus, setSelectedStatus] = useState<string>(statusFilter);
  const [selectedPriority, setSelectedPriority] = useState<string>(priorityFilter);
  ...
  }, [dispatch, dateRange, selectedStatus, selectedPriority]);
  ...
  // Filter orders locally (already filtered from backend, but can apply additional client-side filtering)
    ...
    <div className="space-y-6" ref={reportRef}>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-slate-900">Orders Report (Redux)</h2>
            <p className="text-slate-600">Complete order tracking and management with Redux</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Select value={selectedPriority} onValueChange={handlePriorityFilterChange}>
            <SelectTrigger className="w-full sm:w-[200px]">
              <div className="flex items-center gap-2">
                <Filter className="w-4 h-4" />
