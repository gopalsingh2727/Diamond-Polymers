...
import { fetchProductionReport } from "../../../../../redux/reports/reports/reportActions";
...
export function ProductionReport() {
  ...
  const { production, loading, error } = useSelector((state: RootState) => state.reports);
  const ordersData = production?.orders || [];
  const materialsData = production?.materials || [];
  const productionOutputData = production?.productionOutput || [];
  ...
  useEffect(() => {
    if (dateRange.from && dateRange.to) {
      ...
      if (selectedMaterialType !== "all") filters.materialType = selectedMaterialType;
      dispatch(fetchProductionReport(filters) as any);
    }
  }, [dispatch, dateRange.from, dateRange.to, selectedMaterialType]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-slate-600">Loading production report...</div>
      </div>
    );
  }
  ...
  const totalProduction = filteredOrders.reduce((sum: number, o: any) => sum + (o.realTimeData?.totalNetWeight || 0), 0);
  const totalWastage = filteredOrders.reduce((sum: number, o: any) => sum + (o.realTimeData?.totalWastage || 0), 0);
  const totalMaterialInput = filteredOrders.reduce((sum: number, o: any) => sum + (o.materialWeight || 0), 0);
  ...
  const dailyData = productionOutputData.filter(item => {
  ...
  const materialTypes = Array.from(new Set(materialsData.map(m => m.materialType)));
  ...
  const filteredMaterialsByType = selectedMaterialType === "all"
    ...
    : productionByMaterial.filter((mat: any) => {
        const materialInfo = materialsData.find(m => m.materialName === mat.materialName);
  ...
  const handleExportExcel = () => {
    ...
    const materialData = productionByMaterial.map((m: any) => {
      const materialInfo = materialsData.find(mat => mat.materialName === m.materialName);
      return {
            ...
            <table className="w-full">
              ...
              <tbody>
                {productionByMaterial.map((material, index) => {
                  const materialInfo = materialsData.find(m => m.materialName === material.materialName);
                  return (
