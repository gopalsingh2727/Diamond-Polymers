...
export function CustomerReport() {
  ...
  const customerData = Object.values(
    filteredOrders.reduce((acc, order) => {
      ...
      acc[customerId].totalWeight += order.realTimeData.totalNetWeight || 0;
      acc[customerId].totalWastage += order.realTimeData.totalWastage || 0;
      
      // Track materials used by customer
      const materialType = order.materialType || 'Unknown';
      if (!acc[customerId].materials[materialType]) {
        acc[customerId].materials[materialType] = 0;
      }
      acc[customerId].materials[materialType] += order.realTimeData.totalNetWeight || 0;
      
      if (order.realTimeData.overallEfficiency > 0) {
        acc[customerId].avgEfficiency += order.realTimeData.overallEfficiency;
      }
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-slate-900 truncate">
              {topCustomer ? topCustomer.name.split(' ')[0] : 'N/A'}
            </div>
