...
import { mockMachines, mockMachineUtilization } from "../../lib/mockData";
...
import { getDefaultDateRange, isDateInRange } from "../../lib/dateUtils";
...
export function MachineReport() {
  ...
  // Note: Machine data is not filtered by date range as it represents current state
  // The date filter is available for consistency across reports
  ...
  const avgUtilization = mockMachineUtilization.reduce((sum, m) => sum + m.utilization, 0) / mockMachineUtilization.length;

  // Machine type breakdown
  const machineTypeBreakdown = Object.values(
    mockMachines.reduce((acc, machine) => {
  ...
  const utilizationByType = Object.values(
    mockMachineUtilization.reduce((acc, machine) => {
      const type = machine.machineType;
      if (!acc[type]) {
        acc[type] = {
          type,
  ...
  const topMachines = [...mockMachineUtilization]
    .filter(m => m.status === 'active')
    .sort((a, b) => b.utilization - a.utilization)
    .slice(0, 10);

  // Low performing machines (by utilization, excluding maintenance)
  const lowMachines = [...mockMachineUtilization]
    .filter(m => m.status === 'active')
  ...
  const machineTypes = Array.from(new Set(mockMachines.map(m => m.machineType)));
  
  // Filter machines by selected type
  const filteredMachinesByType = selectedMachineType === "all" 
    ? mockMachineUtilization 
    : mockMachineUtilization.filter(m => m.machineType === selectedMachineType);
  ...
  const handleExportExcel = () => {
    ...
    const summaryData = mockMachineUtilization.map(m => ({
      'Machine': m.machine,
      'Type': m.machineType,
