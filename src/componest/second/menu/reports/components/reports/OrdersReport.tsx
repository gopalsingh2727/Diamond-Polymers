...
export function OrdersReport() {
  ...
  let filteredOrders = filterOrdersByDateRange(mockOrders, dateRange);

  // Filter by status
  if (selectedStatus !== "all") {
    filteredOrders = filteredOrders.filter(order => order.overallStatus === selectedStatus);
  }

  // Filter by priority
  if (selectedPriority !== "all") {
    filteredOrders = filteredOrders.filter(order => order.priority === selectedPriority);
  }
  ...
  const COLORS = ["#10b981", "#3b82f6", "#f59e0b", "#8b5cf6", "#6366f1", "#ef4444"];
