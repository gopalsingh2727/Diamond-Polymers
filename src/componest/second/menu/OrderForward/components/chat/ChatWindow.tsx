...
import { sendChatMessage } from '../../../../../redux/chat/chatActions';
...
const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  const [currentCallId, setCurrentCallId] = useState<string | null>(null); // 🔥 Track callId for all calls
    ...
    try {
      ...
      webrtcService.setEventHandlers({
        ...
        onIceCandidate: (candidate) => {
          ...
          if (wsClient && currentCallId) {
            wsClient.send({
              action: 'call:ice_candidate',
              data: {
                callId: currentCallId, // 🔥 Include callId for tracking
  ...
  const handleAcceptCall = async () => {
    ...
    try {
      ...
      webrtcService.setEventHandlers({
        ...
        onIceCandidate: (candidate) => {
          const wsClient = getWebSocketClient();
          if (wsClient && incomingCall.callId) {
            wsClient.send({
              action: 'call:ice_candidate',
              data: {
                callId: incomingCall.callId, // 🔥 Include callId for tracking
  ...
  const handleRejectCall = () => {
    ...
    setIncomingCall(null);
    setCurrentCallId(null); // 🔥 Clear callId
  ...
  useEffect(() => {
    ...
    const handleCallIncoming = (message: any) => {
      ...
      // WebSocket message structure: { type: 'call:incoming', data: { callId, callerId, callerName, offer, callType } }
      ...
      setIncomingCall({
        callId: data.callId, // 🔥 Include callId
    ...
    // 🔥 NEW: Handle call initiated (with callId)
    const handleCallInitiated = (message: any) => {
      console.log('[Call] Call initiated:', message);
      const data = message.data || message;
      // Store callId for outgoing call
      if (data.callId) {
        setCurrentCallId(data.callId);
        console.log(`[Call] ✅ CallId stored for tracking: ${data.callId}`);
      }
    };

    // 🔥 NEW: Handle busy status
    const handleCallBusy = (message: any) => {
      console.log('[Call] User is busy:', message);
      const data = message.data || message;
      alert(`${chatPerson?.name || 'User'} is currently on another call. Please try again later.`);
      handleEndCall();
    };

    // Register WebSocket event listeners
    wsClient.on('call:incoming', handleCallIncoming);
    wsClient.on('call:initiated', handleCallInitiated); // 🔥 NEW
    wsClient.on('call:busy', handleCallBusy); // 🔥 NEW
    ...
    return () => {
      ...
      wsClient.off('call:initiated', handleCallInitiated); // 🔥 NEW
      wsClient.off('call:busy', handleCallBusy); // 🔥 NEW
