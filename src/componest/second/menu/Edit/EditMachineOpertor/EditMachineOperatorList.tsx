/**
 * EditMachineOperatorList - LIST ONLY
 * Click → Goes to CreateMachineOperator for editing
 */
...
const EditMachineOperatorList: React.FC<Props> = ({ onEdit }) => {
  ...
  const filteredItems = operators.filter((op: Operator) => {
    ...
    const machine = machines.find((m: any) => m._id === op.machineId);
    return (
  ...
  const getMachineName = (machineId: string) => {
    const machine = machines.find((m: any) => m._id === machineId);
    return machine?.machineName || "N/A";
  };
    ...
    <div className="EditMachineType">
      <div className="editsectionsTable-container">
        <div className="editsectionsTable-searchWrapper">
          ...
          <div className="editsectionsTable-countBadge">{filteredItems.length} operators</div>
        </div>
        ...
        {filteredItems.length > 0 ? (
          <div className="editsectionsTable-wrapper">
            <table className="editsectionsTable-table">
              ...
              <tbody className="editsectionsTable-tbody">
                {filteredItems.map((item: Operator, index: number) => (
                  <tr
                    key={item._id}
                  ...
                  >
                    ...
                    <td className="editsectionsTable-td">{item.username}</td>
                    <td className="editsectionsTable-td">{getMachineName(item.machineId)}</td>
          ...
          <div className="editsectionsTable-empty">No operators found</div>
        )}
      </div>
    </div>
  );
...
export default EditMachineOperatorList;
