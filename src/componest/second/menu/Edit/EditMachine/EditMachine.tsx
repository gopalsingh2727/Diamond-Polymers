...
//   const { machines = [], loading, error } = useSelector((state: RootState) => state.machineList);
//   const { items: machineTypes = [] } = useSelector((state: RootState) => state.machineTypeList);
//   const { success: updateSuccess } = useSelector((state: RootState) => state.machineUpdate);
//   const { success: deleteSuccess } = useSelector((state: RootState) => state.machineDelete);
...
//                 onKeyDown={(e) => handleKeyDown(e, index, machine)}
...
const EditMachine: React.FC = () => {
  return (
    <div className="EditMachine p-4">
      <h2 className="text-xl font-semibold mb-4">Edit Machine</h2>
      <p>This component is under construction. Please check back later.</p>
    </div>
  );
}
